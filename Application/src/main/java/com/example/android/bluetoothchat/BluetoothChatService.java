...
public class BluetoothChatService extends Service{
    ...
    private void connectionLost(int num) {
        ...
        if(multiBT.isEmpty()){
            if(BluetoothChatFragment.getState() == BluetoothChatFragment.dualmode){
                ...
                BluetoothChatFragment.setState(BluetoothChatFragment.idle);
            }
        }
    ...
    private class ConnectedThread extends Thread {
        ...
        public void run() {
            ...
            sendRoomList(BluetoothChatFragment.RoomList);
            ...
            if(BluetoothChatFragment.getState() == BluetoothChatFragment.internet){
                ...
                BluetoothChatFragment.setState(BluetoothChatFragment.bt);
            }
        ...
        public synchronized boolean sendSync(String message) {
            try {
                //byte[] buffer = (BTname+"##"+message).getBytes();
                byte[] buffer = (message).getBytes();
                mmOutStream.write(buffer);
        ...
        public void sendRoomList(ArrayList<String> roomlist){
            Log.d(TAG, "SendRoomlist");
            String tmp = "ChatList";
            if(roomlist.isEmpty()){
                return ;
            }
            for(String n : roomlist){
                tmp += "##" + n;
            }
            this.write(tmp);
        }
    ...
    public void scanstart(){
        autoing = false;
        BluetoothChatFragment.delFriend();
        startScanning();
    }
    ...
    private class SampleScanCallback extends ScanCallback {
        ...
        public void onScanResult(int callbackType, ScanResult result) {
            ...
            if(!scanresult.contains(result.getDevice().getName()+"\n"+result.getDevice().getAddress())){
                scanresult.add(result.getDevice().getName()+"\n"+result.getDevice().getAddress());
                BluetoothChatFragment.addFriend(result.getDevice().getName() + "\n" + result.getDevice().getAddress());
                BluetoothChatFragment.FriendlistAdapter.add(result.getDevice().getName() + "\n" + result.getDevice().getAddress());
            }
            try{
                Log.d(TAG, new String(result.getScanRecord().getServiceData(Constants.Service_UUID)));
                scanaddress = new String(result.getScanRecord().getServiceData(Constants.Service_UUID));
